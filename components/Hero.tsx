...
const Hero: React.FC = () => {
  ...
  const HeroImage = ({ className = "" }: { className?: string }) => (
    <div className={`hero-visual-area relative ${className}`}>
      <div className="floating-blob absolute -top-10 -left-10 w-40 h-40 bg-[#E27D37]/10 rounded-full blur-3xl -z-10"></div>
      <div className="floating-blob absolute -bottom-10 -right-10 w-32 h-32 bg-[#E27D37]/5 rounded-full blur-2xl -z-10"></div>
      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          <div className="w-full lg:w-3/5 text-center lg:text-left z-10">
            <div className="hero-line-static inline-block mb-6 px-4 py-1.5 rounded-full bg-white border border-[#E27D37]/30 text-[#E27D37] font-bold text-xs tracking-widest shadow-sm">
            ...
            <h1 className="text-[#3D3B3A] leading-tight mb-8">
              ...
              <span className="block text-4xl md:text-5xl lg:text-6xl font-black text-[#E27D37] mt-8 flex items-center justify-center lg:justify-start">
                {brandText.split("").map((char, i) => (
                  <span key={i} className="typing-letter" style={{ whiteSpace: char === " " ? "pre" : "normal" }}>
                    {char}
                  </span>
                ))}
                <span className="typing-cursor ml-1 inline-block w-1.5 h-10 md:h-12 lg:h-14 bg-[#E27D37]"></span>
              </span>
            </h1>
