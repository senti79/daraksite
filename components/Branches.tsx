                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
                    {Object.entries(branchesData).map(([district, branches]) => (
                        <div key={district} className="sketch-card bg-white p-8 pt-24 relative overflow-hidden">
                            {/* Decorative Top Bar */}
                            <div className="absolute top-0 left-0 w-full h-10 bg-[#D35400]/30 flex justify-center items-center gap-4 border-b border-[#D35400]/25">
                                {[1, 2, 3, 4, 5, 6].map((i) => (
                                    <div key={i} className="w-2.5 h-2.5 rounded-full bg-white border border-[#D35400]/40 shadow-inner"></div>
                                ))}
                            </div>



                            <h3 className="text-2xl font-bold text-[#D35400] mb-6 border-b-2 border-[#D35400] pb-2">{district}</h3>
