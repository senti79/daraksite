...
const branchesData = {
  ...
  '대전 서구': [
    ...
    { name: '도안 호수원', phone: '010-4071-2939' },
  ...
  '대전 유성구': [
    ...
    { name: '관평원', phone: '010-5485-7928' },
    { name: '계산원', phone: '010-5680-6988' },
  ],
  '대전 중구/동구/대덕구': [
    { name: '용두원', phone: '010-3055-8945' },
    { name: '중리원', phone: '010-8448-9993' },
  ],
  세종: [{ name: '나성원', phone: '010-5656-1959' }],
};
    ...
    <section id="branches" className="py-20 md:py-32 bg-white">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-[#3D3B3A]">
            <span className="text-highlight">가까운 다락서원 찾기</span>
          </h2>
          <p className="mt-4 text-lg text-gray-600 ">우리 동네 독서 전문 교육원</p>
          <br className="md:hidden" />
          <p className="mt-1 text-sm text-gray-400 md:hidden font-medium">
            연락처를 클릭하면 바로 연결됩니다.
          </p>
        </div>
            ...
            <div key={district} className="sketch-card bg-white p-8 pt-24 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-10 bg-[#065F46]/25 flex justify-center items-center gap-4 border-b border-[#065F46]/20">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className="w-2.5 h-2.5 rounded-full bg-white border border-[#065F46]/40 shadow-inner"
                  />
                ))}
              </div>

              <h3 className="text-2xl font-bold text-[#D35400] mb-6 border-b-2 border-[#D35400] pb-2">
                {district}
              </h3>
                ...
                {branches.map((branch) => (
                  <li key={branch.name} className="flex justify-between items-center">
                    ...
                    <a
                      ...
                      className="text-gray-600 hover:text-[#D35400] transition-colors tabular-nums"
                    >
                      {branch.phone}
                    </a>
                  </li>
