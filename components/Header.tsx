    ...
    <header className="bg-[#F8F5F2] sticky top-0 z-[100] border-b border-gray-200/50 backdrop-blur-sm bg-[#F8F5F2]/90">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="#" onClick={(e) => {
          ...
          <span className="font-black text-xl md:text-2xl text-[#3D3B3A] tracking-tighter transition-colors group-hover:text-[#F47920]">
            책읽는 다락서원
          </span>
        </a>
        ...
        <nav className="hidden md:flex space-x-8">
          {navLinks.map(link => (
            <a
              ...
              className="text-gray-600 hover:text-[#F47920] transition-colors duration-300 font-bold text-sm tracking-tight"
            >
              {link.label}
            </a>
          ))}
      ...
      <div className={`md:hidden fixed top-0 left-0 w-full h-screen bg-[#F8F5F2] z-50 transform transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-8">
          {navLinks.map(link => (
            <a key={link.href} href={link.href} onClick={(e) => handleLinkClick(e, link.href)} className="text-2xl font-bold text-gray-800 hover:text-[#F47920] transition-colors duration-300">{link.label}</a>
          ))}
        </div>
      </div>
    </header>
